...
- 新增“智能选择缓存/临时文件”入口：会自动选中已通过后端安全判定的缓存、临时目录、下载目录安装包等低风险候选；该按钮只负责选择和创建确认，不绕过后端 dry-run、风险确认、备份、执行后验证和审计。
- 任务页的风险确认区域新增“确认”和“确认并执行”，用户不再必须跳转到风险确认中心才能完成当前任务的清理闭环。
- 清理执行成功后，页面新增“本次已清理”汇总，按候选冻结时的 `size_bytes` 展示“已清理文件”和“释放约多少磁盘空间”；后端生产执行结果仍以目标消失、备份哈希和审计记录作为真实验证依据。
