...
只读：`capability_probe`、`system_snapshot`、`memory_snapshot`、`process_list`、`zombie_process_scan`、`network_socket_list`、`port_owner_lookup`、`disk_usage_scan`、`filesystem_inventory`、`large_file_scan`、`open_file_lookup`、`journal_query`、`service_status`、`config_drift_check`、`io_diagnose`、`network_config_snapshot`、`package_inventory`、`scheduled_task_inventory`、`login_audit`、`kernel_log_query`、`security_baseline_scan`。

新增只读运维场景覆盖：

- 内存与 Swap：`memory_snapshot`
- 挂载点、容量与 inode：`filesystem_inventory`
- 路由、网卡地址与 DNS：`network_config_snapshot`
- RPM 软件包版本与架构：`package_inventory`
- crontab、cron.d 与 systemd timer：`scheduled_task_inventory`
- 最近登录审计：`login_audit`
- 内核 warning 日志：`kernel_log_query`

以上工具均为固定实现、固定 argv 或受控只读文件读取，不提供通用 Shell，不执行状态变更；输出统一脱敏、截断，并作为 `UNTRUSTED_DATA` 进入 Agent。
