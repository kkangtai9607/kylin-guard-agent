...
TOOL_DESCRIPTIONS = {
    ...
    "memory_snapshot": ("内存快照", "读取内存、Swap 与关键 meminfo 指标", RiskLevel.L1),
    ...
    "filesystem_inventory": ("文件系统盘点", "读取挂载点、容量和 inode 概况", RiskLevel.L2),
    "large_file_scan": ("大文件扫描", "在受控目录内扫描大文件", RiskLevel.L2),
    ...
    "network_config_snapshot": ("网络配置快照", "读取路由、网卡地址和 DNS 解析配置", RiskLevel.L2),
    "package_inventory": ("软件包盘点", "读取 RPM 软件包名称、版本和架构", RiskLevel.L2),
    "scheduled_task_inventory": ("计划任务盘点", "读取 crontab、cron.d 文件名和 systemd timer", RiskLevel.L2),
    "login_audit": ("登录审计", "读取最近登录记录用于账号活动诊断", RiskLevel.L2),
    "kernel_log_query": ("内核告警日志", "读取内核 warning 级别日志", RiskLevel.L2),
